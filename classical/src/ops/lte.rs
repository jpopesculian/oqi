...
impl Value {
    pub fn lte_(self, rhs: Self) -> Result<Self> {
        Lte::checked_op(self, rhs)
    }
}
...
mod tests {
    ...
    #[test]
    fn bool_lte_is_err() {
        assert!(bool_scalar(true).lte_(bool_scalar(true)).is_err());
    }

    #[test]
    fn int_float_lte_via_lt() {
        let r = i_scalar(3, 8)
            .lte_(Value::Scalar(Scalar::new_unchecked(
                Primitive::float(3.5),
