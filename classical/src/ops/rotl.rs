...
mod tests {
    ...
    fn rotl_bitreg_by_3() {
        ...
        match r {
            Value::Scalar(s) => {
                assert!(matches!(s.ty(), BitReg(w) if w == 8));
    ...
    fn rotl_uint_by_1() {
        ...
        match r {
            Value::Scalar(s) => {
                ...
                assert_eq!(s.value().as_uint(iw(8)).unwrap(), 0b0000_0011);
            }
            _ => panic!("expected scalar"),
        }
    }
    ...
    fn rotl_with_uint_distance() {
        ...
        match r {
            Value::Scalar(s) => {
                assert_eq!(s.value().as_bitreg(4).unwrap().as_u128(), 0b0010);
            }
            _ => panic!("expected scalar"),
        }
    }
