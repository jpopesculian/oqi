...
use oqi_compile::bytecode::{BcModule, emit, is_sample_safe};
...
#[test]
fn sample_safe_accepts_unitary_then_measure() {
    // Straight-line Bell, and the control-flow GHZ (gates before measure, even
    // with `if`/`for`) are both safe to sample from one final state.
    assert!(is_sample_safe(&build(
        r#"OPENQASM 3.0; include "stdgates.inc";
           qubit[2] q; bit[2] c;
           h q[0]; cx q[0], q[1];
    ...
    assert!(is_sample_safe(&build(
        r#"OPENQASM 3.0; include "stdgates.inc";
           input int size; qubit[8] q; bit[8] c;
           if (size > 0) { h q[0]; }
           for int i in [1:size - 1] { cx q[i - 1], q[i]; }
           c = measure q;"#
    )));
}

#[test]
fn sample_safe_rejects_feedback() {
    // A gate conditioned on a measurement is feedback — not sample-safe.
    assert!(!is_sample_safe(&build(
        r#"OPENQASM 3.0; include "stdgates.inc";
           qubit[2] q;
           h q[0];
